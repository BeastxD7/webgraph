...
class MainContentConfig:
    ...
    adaptive_cost: bool = config.CONTENT_ADAPTIVE_COST
    ...
    **+0.035 WCXB, -0.019 Zyte** -- and the trade is not close. Adaptive wins on **all seven**
    ...
    cost_ceiling: float = config.CONTENT_COST_CEILING
    ...
    heading_bonus: float = config.CONTENT_HEADING_BONUS
    ...
    structural_credit: float = config.CONTENT_STRUCTURAL_CREDIT
    ...
    **Measured as inert, and kept only pending a corpus that can see it.** Swept 0.0, 0.25,
    ...
    bridge: float = config.CONTENT_BRIDGE
    ...
    trust_main_links: bool = config.CONTENT_TRUST_MAIN_LINKS
    ...
    **Measured twice, with opposite verdicts, and the second is the one that counts.** The
    ...
    group_repeats: str = config.CONTENT_GROUP_REPEATS
    ...
    min_group_size: int = config.CONTENT_MIN_GROUP_SIZE
    """Fewer repeated siblings than this is not a grid."""

    group_min_share: float = config.CONTENT_GROUP_MIN_SHARE
