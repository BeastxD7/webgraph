(markers) => {
  ...
  for (const el of nodes) {
    ...
    // A fixed box parked *beyond* the viewport -- a cart drawer at `translateX(100%)`, a
    // size-chart panel that slides in from the right -- is not marked hidden, and this was
    // measured both ways. Marking it `offscreen` cleaned allbirds.com's product page of
    // its empty cart drawer and cost twinssslippers.com's product page 60% of its words:
    // that store keeps the product description, the size chart, delivery and payment in
    // slide-in panels a button opens, and Chromium's own text holds every one of them. An
    // off-canvas panel is a collapsed tray, not a spam div at -2e13px; a reader reaches it
    // with one click, and completeness is the promise. It keeps its box and is ordered
    // where the box says, after the page.
