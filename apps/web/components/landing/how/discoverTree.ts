/**
 * One shared timeline, in milliseconds, for step 1's whole intro cycle: blank canvas, the
 ...
 * turns all of it into real `@keyframes` text; `Step1Intro.tsx` injects that once and points
...
/** Playback pace for the whole cycle -- the intro and the reveal together, since every
 * keyframe above and below is a fraction of one shared duration. 1 is the pace the
 * playground was tuned at (brisk); 1.6 is the "medium" the landing page plays at, slow
 * enough to follow the typing and watch each level of the crawl land. */
...
export const TIMING = {
  ...
  edgeDrawMs: 80,
  nodePopMs: 90,
  holdMs: 1100, // the finished tree rests, readable, before it dissolves
  fadeMs: 200,
  tailMs: 180,
} as const;
 ...
 * discovered page. The cycle ends by fading the tree back out, so an `infinite` animation
 * loops cleanly: build, hold, dissolve, build again. */
