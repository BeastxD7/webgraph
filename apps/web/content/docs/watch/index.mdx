...
The first run is the **baseline** and records no changes. `removed` is claimed only when
the site said so: a page the previous run read that this run asked for and got an HTTP 4xx
back. A page the run did not reach — the page cap was hit first — is **unverified**, not
gone; a page that failed for another reason (a timeout, a 500) is neither.
