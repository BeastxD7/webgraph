...
| Metadata | JSON-LD `@type`s in the plain HTML (vercel.com's arrive with JavaScript, so the plain fetch sees none -- the gap the report exists to show), OpenGraph / Twitter card counts, `hreflang` count, `rel=canonical` | the root's plain HTML |
