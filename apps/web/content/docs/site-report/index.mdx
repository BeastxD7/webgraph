...
`Disallow` lines that decide something, any `Crawl-delay`, and the lines verbatim. Every
one of the report's own requests identifies itself as webgraph and obeys the file like
every other fetch the engine makes; it is spaced `REPORT_REQUEST_INTERVAL_SECONDS` (1 s)
apart per host.
...
version and commit, the User-Agent used, pages sampled, the request interval, the duration,
and the statement that nothing was fetched as another bot.
