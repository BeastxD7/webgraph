...
| `render_note` | Set when the browser was shown a gate that hid the page -- one hidden element holding more of the page's own prose than the whole visible render (a country picker, a consent dialog with the body set `display: none` behind it) -- and the plain fetch's page was kept whole behind it. `null` otherwise; a hidden menu or a collapsed section is not a gate and stays hidden |
| `strategy`, `static_chars`, `rendered_chars` | How this page was fetched, and what each fetch gave in characters of readable text. A `union` with `rendered_chars` 0 is the plain fetch's page alone: the browser produced an empty page (a WebGL scene in a headless browser does) |
